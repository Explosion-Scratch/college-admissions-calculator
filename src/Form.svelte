<script>
  ...
  export const setValue = async function setValue(new_val) {
    let els = [...container.querySelectorAll(INPUT_QS)];

    for (let [k, v] of Object.entries(new_val)) {
      let el = els.find((i) => i.name == k || i.getAttribute("name") == k);
      setValue(el, v);
    }
    function setValue(el, v) {
      // TODO: Separate input logic and for each input type have a getValue and setValue;
      el.value = v;
    }
    let e = new CustomEvent("run-validate");
    [...container.querySelectorAll(".container")].forEach((i) =>
      i.dispatchEvent(e)
    );
  ...
  function getValue(input) {
    ...
    if (input.matches(".radio_container")) {
      return [...input.querySelectorAll("input[type=radio]")].find(
        (i) => i.checked
      ).value;
    }
...
>
  ...
  {#if description}<label for="{id}" class="description {type}_description"
      >{description}</label
    >{/if}
  {#if !VALID && type !== "form"}
    <span class="error"
      >{ERROR_MESSAGE || errorMessage || DEFAULT_INVALID_MESSAGE}</span
    >
  {/if}
  {#if type === "input"}
      ...
      {#each options as option}
        <label
          ><input
            ...
            {name}
            value="{option.value || option}"
            checked="{value.includes(options.value || option) || null}"
          />{option.label || option}</label
        >
      {/each}
    </div>
